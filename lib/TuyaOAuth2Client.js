...
class TuyaOAuth2Client extends OAuth2Client {
  ...
  static CLIENT_ID = 'dummy';
  static CLIENT_SECRET = 'dummy';
  static API_URL = 'https://tuya.athom.com/api';
  static TOKEN_URL = 'https://tuya.athom.com/api/v1.0/token';
  static AUTHORIZATION_URL = 'https://tuya.athom.com/authorise';
  ...
  async onRequestHeaders({ headers }) {
    headers = await super.onRequestHeaders({ headers });

    const token = await this.getToken();
    headers['x-tuya-region'] = token.region;
  ...
  async onGetTokenByCode({ code }) {
    ...
    const response = await fetch(`${this._tokenUrl}?code=${payload.code}&grant_type=${TuyaOAuth2Constants.GRANT_TYPE.OAUTH2}`, {
      headers: {
        'x-tuya-region': payload.region,
      },
    });
