  ...
  async onOAuth2Init() {
    ...
    const switchAutocompleteListener = (query, args) => {
      ...
      return tuyaSwitches.map(value => {
        const name = `Switch ${value.substring(7)}`;
        return {
          name: name,
          id: value,
        };
    ...
    this.homey.flow.getActionCard('sub_switch_on')
      .registerArgumentAutocompleteListener('switch', (query, args) => switchAutocompleteListener(query, args));

    this.homey.flow.getActionCard('sub_switch_off')
      .registerArgumentAutocompleteListener('switch', (query, args) => switchAutocompleteListener(query, args));

    this.homey.flow.getDeviceTriggerCard('sub_switch_turned_on')
      .registerArgumentAutocompleteListener('switch', (query, args) => switchAutocompleteListener(query, args))
      .registerRunListener((args, state) => args.switch.id === state.tuyaCapability);

    this.homey.flow.getDeviceTriggerCard('sub_switch_turned_off')
      .registerArgumentAutocompleteListener('switch', (query, args) => switchAutocompleteListener(query, args))
      .registerRunListener((args, state) => args.switch.id === state.tuyaCapability);

    this.homey.flow.getConditionCard('sub_switch_is_on')
      .registerArgumentAutocompleteListener('switch', (query, args) => switchAutocompleteListener(query, args))
