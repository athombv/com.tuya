  ...
  async onOAuth2Init() {
    ...
    if (this.hasCapability('onoff')) {
      this.registerCapabilityListener('onoff', (value) =>
        this.onOffCapabilityListener(value)
      );
    }

    if (this.hasCapability('target_temperature')) {
      this.registerCapabilityListener('target_temperature', (value) =>
        this.targetTemperatureCapabilityListener(value)
      );
    }

    if (this.hasCapability('child_lock')) {
      this.registerCapabilityListener('child_lock', (value) =>
        this.childLockCapabilityListener(value)
      );
    }

    if (this.hasCapability('eco_mode')) {
      this.registerCapabilityListener('eco_mode', (value) =>
        this.ecoModeCapabilityListener(value)
      );
    }
  }
  ...
  async onTuyaStatus(status) {
    ...
    if (typeof status['temp_current'] === 'number') {
      this.setCapabilityValue(
        'measure_temperature',
        status['temp_current'] > 100 || status['temp_current'] < 100
          ? status['temp_current'] / 10
          : status['temp_current']
      ).catch(this.error);
    }

    if (typeof status['temp_set'] === 'number') {
      this.setCapabilityValue('target_temperature', status['temp_set']).catch(
        this.error
      );
    }
